...
- (void)drawRect:(CGRect)rect {
    ...
    if (([[self dataD] count] > 0) && ([self style] != kRegionPlotNone)) {
        CGContextSaveGState(myContext);
        WSDatum *pointD = [[self dataD] dataAtIndex:0];
        CGContextMoveToPoint(myContext, 
                             [self boundsWithDataX:[pointD valueX]], 
                             [self boundsWithDataY:[pointD value]]);
