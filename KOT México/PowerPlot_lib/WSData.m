...
/// Add the methods for data bindings support.
@interface WSData ()
...
+ (id)dataWithValues:(NSArray *)vals
         withValuesX:(NSArray *)valsX
       withErrorMinY:(NSArray *)errMinY
       withErrorMaxY:(NSArray *)errMaxY
       withErrorMinX:(NSArray *)errMinX
       withErrorMaxX:(NSArray *)errMaxX {
    return [[[self alloc] initWithValues:vals
                             withValuesX:valsX
                           withErrorMinY:errMinY
                           withErrorMaxY:errMaxY
                           withErrorMinX:errMinX
                           withErrorMaxX:errMaxX]
            autorelease];
}
...
- (WSData *)indexedData {
    ...
    for (i=0; i<[indexed count]; i++) {
        [[indexed dataAtIndex:i] setValueX:(NAFloat)i];
    }
...
- (id)initWithValues:(NSArray *)vals {
    return [self initWithValues:vals
                    withValuesX:[WSData arrayOfZerosWithLen:[vals count]]];
}
    ...
    if (self) {
        ...
        for (i=0; i<[vals count]; i++) {
            WSDatum *tmp = [WSDatum datumWithValue:[[vals objectAtIndex:i]
                                                    floatValue]
                                    withAnnotation:[annos objectAtIndex:i]];
...
- (id)initWithValues:(NSArray *)vals
    ...
    if (self) {
        ...
        for (i=0; i<[vals count]; i++) {
            WSDatum *tmp = [WSDatum datumWithValue:[[vals objectAtIndex:i]
                                                    floatValue]
                                        withValueX:[[valsX objectAtIndex:i]
                                                    floatValue]];
            if ([errMinY count] > 0) {
...
- (void)insertData:(WSDatum *)aDatum
...
- (void)replaceDataAtIndex:(NSUInteger)index
                  withData:(WSDatum *)aDatum {
    [self willChangeValueForKey:KVO_VALUES];
...
- (NAFloat)minimumValue {
    ...
    if ([self count] > 0) {
        retVal = [[self dataAtIndex:0] value];
