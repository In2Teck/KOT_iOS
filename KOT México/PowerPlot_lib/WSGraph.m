...
+ (id)graphWithNodes:(WSData *)nodes
     withConnections:(WSGraphConnections *)connections {
...
- (id)init {
    return [self initWithNodes:[WSData data]
               withConnections:[[WSGraphConnections new]
                                autorelease]];
}
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    [copy setConnections:[[[self connections] copy] autorelease]];
    return copy;
}
...
- (void)setAllNodesTo:(WSNodeProperties *)custom {
    for (WSDatum *item in [self values]) {
        [item setCustomDatum:[[custom copy] autorelease]];
    }    
}
...
- (void)insertNode:(WSDatum *)node
    ...
    [super insertData:node atIndex:index];
}

- (void)replaceNodeAtIndex:(NSUInteger)index
                  withNode:(WSDatum *)node {
    [super replaceDataAtIndex:index withData:node];
}
...
- (void)removeNodeAtIndex:(NSUInteger)index {
    [super removeDataAtIndex:index];
}

- (WSDatum *)nodeAtIndex:(NSUInteger)index {
    return (WSDatum *)[super dataAtIndex:index];
}
